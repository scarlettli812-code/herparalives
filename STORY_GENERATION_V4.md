...
首章生成使用 120 秒、单次尝试的硬预算，输出最多 2800 tokens：首次只返回五章简要大纲与一个可玩的关键决策场景；玩家做出选择后，后续章节恢复完整叙事密度。模型超时后，接口会在 Vercel 300 秒截止前返回透明标记的安全模板，不再因第二次长重试被平台终止为 504。后续章节允许两次 110 秒尝试，总预算仍保留平台响应余量。
